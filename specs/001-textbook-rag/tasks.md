...
- [X] T032 [US2] Create backend/src/api/chat.py with POST /chat endpoint orchestrating retrieval → generation → response
...
```
              ...
              ▼              ▼              ▼
    ┌─────────────────┐ ┌────────────┐ ┌────────────┐
    │ Foundational    │ │   US1      │ │   US1      │
    │ (Backend only)  │ │ (Content)  │ │ (Frontend) │
    └────────┬────────┘ └─────┬──────┘ └─────┬──────┘
             │                │              │
             │                ▼              │
             │       ┌────────────────┐      │
